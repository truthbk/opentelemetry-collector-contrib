...
- `capabilities`: Keys with boolean true/false values that enable a particular OpAMP capability.
  ...
  - `requires_payload_trust_verification`: Whether to enable the OpAMP RequiresPayloadTrustVerification capability. Default is `false`. When set, every `ServerToAgent` message from the server is wrapped in a `SignedServerToAgent` envelope; the extension validates the X.509 chain on the first message of each connection and verifies the detached signature on every subsequent message against `signing.ca_cert_file`. Must be set together with `signing.ca_cert_file`.
- `signing`: PKI material for OpAMP Message Attestation. Required when `capabilities.requires_payload_trust_verification` is `true`; ignored otherwise.
  - `ca_cert_file`: Filesystem path to a PEM-encoded X.509 CA bundle. Every certificate in the bundle becomes a trust anchor for chain validation. Multiple anchors are allowed (for example to cover a key-rotation overlap window). When both `signing` and `requires_payload_trust_verification` are absent, the OpAMP wire format is unchanged from when the capability is disabled.
