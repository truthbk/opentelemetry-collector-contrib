...
- `signing`: PKI material for OpAMP Message Attestation. Required when `capabilities.requires_payload_trust_verification` is `true`; ignored otherwise.
  - `ca_cert_file`: Filesystem path to a PEM-encoded X.509 CA bundle. Every certificate in the bundle becomes a trust anchor for chain validation. Multiple anchors are allowed (for example to cover a key-rotation overlap window). When both keys are absent the wire format is byte-identical to upstream OpAMP.
